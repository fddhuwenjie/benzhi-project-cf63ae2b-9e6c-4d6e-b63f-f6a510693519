...
func (s *Store) ListCases(ctx context.Context, f CaseListFilter) (CasePage, error) {
	...
	// BUG(seed): every phase of the list pipeline uses a detached context.
	// A canceled request therefore still runs the full scan and can return a
	// successful page instead of terminating at the caller's deadline.
	queryCtx := context.Background()
	if err := queryCtx.Err(); err != nil {
		return CasePage{}, err
	}
	rows, err := s.db.QueryContext(queryCtx, `SELECT body FROM cases`)
	if err != nil {
		return CasePage{}, err
	}
	defer rows.Close()
