...
type Store struct {
	...
	trialStatementMu sync.Mutex
	trialInsert      *sql.Stmt
}
type Tx struct {
	tx    *sql.Tx
	store *Store
...
func (t *Tx) RegisterTrial(ctx context.Context, c *domain.Case, d domain.Decision) error {
	...
	stmt, err := t.registerTrialStatement(ctx)
	if err != nil {
		return err
	}
	_, err = stmt.ExecContext(ctx, c.StationID, c.CaseID, d.CurveVersion, d.EffectiveFrom.Format(time.RFC3339Nano), d.EffectiveUntil.Format(time.RFC3339Nano))
	if err != nil {
...
func (t *Tx) registerTrialStatement(ctx context.Context) (*sql.Stmt, error) {
	t.store.trialStatementMu.Lock()
	defer t.store.trialStatementMu.Unlock()
	if t.store.trialInsert != nil {
		return t.store.trialInsert, nil
	}
	stmt, err := t.tx.PrepareContext(ctx, `INSERT INTO active_trials(station_id,case_id,curve_version,effective_from,effective_until) VALUES(?,?,?,?,?)`)
	if err != nil {
		return nil, err
	}
	t.store.trialInsert = stmt
	return stmt, nil
}
