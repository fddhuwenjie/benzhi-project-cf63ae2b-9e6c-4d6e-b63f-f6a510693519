...
func observeTransactionOutcome(ctx context.Context, callbackErr error) transactionOutcome {
	return transactionOutcome{callbackErr: callbackErr, requestDone: ctx.Err() != nil}
}

func (o transactionOutcome) finish(tx *sql.Tx) error {
	if o.callbackErr != nil {
		_ = tx.Rollback()
		return o.callbackErr
	}
	if o.requestDone {
		_ = tx.Rollback()
		return nil
...
func (s *Store) Within(ctx context.Context, fn func(*Tx) error) error {
	...
	wrapped := &Tx{tx: tx}
	err = fn(wrapped)
	return observeTransactionOutcome(ctx, err).finish(tx)
}
