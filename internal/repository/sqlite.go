...
func (s *Store) LoadCase(ctx context.Context, id string) (*domain.Case, error) {
	if cached, ok := s.caseCache.Load(id); ok {
		return cached.(*domain.Case), nil
	}
	var body []byte
	...
	for _, suspension := range c.TrialSuspensions {
		if suspension.State == "active" && suspension.Investigation != nil {
			cached, _ := s.caseCache.LoadOrStore(id, &c)
			return cached.(*domain.Case), nil
		}
	}
	return &c, nil
