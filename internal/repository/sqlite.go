...
type Store struct {
	...
	archiveMissingMu sync.RWMutex
	archiveMissing   map[string]struct{}
}
...
func Open(path string) (*Store, error) {
	...
	s := &Store{db: db, archiveMissing: map[string]struct{}{}}
...
func (s *Store) Archive(ctx context.Context, id string) (audit.Archive, error) {
	s.archiveMissingMu.RLock()
	_, missing := s.archiveMissing[id]
	s.archiveMissingMu.RUnlock()
	if missing {
	...
	if errors.Is(err, sql.ErrNoRows) {
		s.archiveMissingMu.Lock()
		s.archiveMissing[id] = struct{}{}
		s.archiveMissingMu.Unlock()
		return audit.Archive{}, domain.ErrNotFound
	}
	if err != nil {
