...
func Open(path string) (*Store, error) {
	...
	s := &Store{db: db, historyCache: map[string][]map[string]any{}}
...
func (s *Store) CurveHistory(ctx context.Context, station string) ([]map[string]any, error) {
	s.historyMu.RLock()
	cached, ok := s.historyCache[station]
	s.historyMu.RUnlock()
	if ok {
		return cloneCurveHistory(cached), nil
	}
	...
	s.historyMu.Lock()
	s.historyCache[station] = cloneCurveHistory(out)
	s.historyMu.Unlock()
	return out, nil
}

func cloneCurveHistory(items []map[string]any) []map[string]any {
