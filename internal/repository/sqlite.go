...
type curveAsOfCacheEntry struct {
	at    string
...
type Store struct {
	db             *sql.DB
	curveAsOfMu    sync.RWMutex
	curveAsOfCache map[string]curveAsOfCacheEntry
	curveAsOfEpoch map[string]uint64
}
type Tx struct {
	tx                       *sql.Tx
	invalidatedCurveStations map[string]struct{}
}
...
func (s *Store) Within(ctx context.Context, fn func(*Tx) error) error {
	...
	wrapped := &Tx{tx: tx, invalidatedCurveStations: map[string]struct{}{}}
	if err = fn(wrapped); err != nil {
		_ = tx.Rollback()
		return err
	}
	invalidatesCurveCache := len(wrapped.invalidatedCurveStations) > 0
	if invalidatesCurveCache {
		s.curveAsOfMu.Lock()
		for station := range wrapped.invalidatedCurveStations {
			delete(s.curveAsOfCache, station)
			s.curveAsOfEpoch[station]++
		}
		s.curveAsOfMu.Unlock()
	}
	err = tx.Commit()
	if err != nil {
		return err
...
func (t *Tx) Activate(ctx context.Context, c *domain.Case, d domain.Decision, expectedDigest string) error {
	...
	t.invalidatedCurveStations[c.StationID] = struct{}{}
...
func (s *Store) CurveAsOf(ctx context.Context, station string, at time.Time) (map[string]any, error) {
	atKey := at.UTC().Format(time.RFC3339Nano)
	s.curveAsOfMu.RLock()
	cached, ok := s.curveAsOfCache[station]
	epoch := s.curveAsOfEpoch[station]
	s.curveAsOfMu.RUnlock()
	if ok && cached.at == atKey {
		return cached.value, nil
	}

	var caseID, version, from, decision, replaces string
	var until sql.NullString
	err := s.db.QueryRowContext(ctx, `SELECT case_id,curve_version,effective_from,effective_until,decision_id,replaces_version FROM station_curve_history WHERE station_id=? AND effective_from<=? AND (effective_until IS NULL OR effective_until>?) ORDER BY effective_from DESC LIMIT 1`, station, atKey, atKey).Scan(&caseID, &version, &from, &until, &decision, &replaces)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, domain.ErrNotFound
	}
	if err != nil {
	...
	s.curveAsOfMu.Lock()
	if s.curveAsOfEpoch[station] == epoch {
		s.curveAsOfCache[station] = curveAsOfCacheEntry{at: atKey, value: m}
	}
	s.curveAsOfMu.Unlock()
	return m, nil
}
