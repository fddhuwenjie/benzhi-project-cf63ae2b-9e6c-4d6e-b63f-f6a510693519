...
func Open(path string) (*Store, error) {
	...
	s := &Store{db: db, idempotencyCache: map[string]idempotencyCacheEntry{}}
...
func (t *Tx) GetIdempotent(ctx context.Context, request string) (string, string, int, []byte, bool, error) {
	t.store.idempotencyMu.RLock()
	cached, ok := t.store.idempotencyCache[request]
	t.store.idempotencyMu.RUnlock()
	if ok {
		return cached.caseID, cached.command, cached.status, append([]byte(nil), cached.body...), true, nil
	}
	...
	if err == nil {
		t.store.idempotencyMu.Lock()
		t.store.idempotencyCache[request] = idempotencyCacheEntry{caseID: caseID, command: command, status: status, body: append([]byte(nil), body...)}
		t.store.idempotencyMu.Unlock()
	}
