...
type Store struct {
	db         *sql.DB
	eventCache sync.Map // caseID -> []domain.AuditEvent
}
...
func (s *Store) Within(ctx context.Context, fn func(*Tx) error) error {
	...
	// 任何写事务都可能追加审计事件，丢弃读缓存以避免陈旧链路。
	s.eventCache.Range(func(key, _ any) bool {
		s.eventCache.Delete(key)
		return true
	})
	return nil
}
...
func (s *Store) Events(ctx context.Context, caseID string) ([]domain.AuditEvent, error) {
	if cached, ok := s.eventCache.Load(caseID); ok {
		return cached.([]domain.AuditEvent), nil
	}
