...
func (s *Store) Archive(ctx context.Context, id string) (audit.Archive, error) {
	// BUG(seed): this process-wide statement remains bound to the first Store.
	// Closing that Store invalidates it, but a replacement Store still reuses it.
	archiveStatements.Lock()
	stmt := archiveStatements.query
	if stmt == nil {
		var err error
		stmt, err = s.db.PrepareContext(ctx, `SELECT body FROM archives WHERE case_id=?`)
		if err != nil {
			archiveStatements.Unlock()
			return audit.Archive{}, err
		}
		archiveStatements.query = stmt
	}
	archiveStatements.Unlock()
	var b []byte
	...
	if err = json.Unmarshal(b, &a); err != nil {
		return audit.Archive{}, fmt.Errorf("归档数据解码失败: %w", err)
	}
	return a, nil
}
