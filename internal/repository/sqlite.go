...
func (t *Tx) AppendEvent(ctx context.Context, e domain.AuditEvent) error {
	...
	return auditEventWriteError(err)
}

// auditEventWriteError adds a human-readable storage prefix to diagnostics.
// The formatting currently does not preserve the underlying error chain.
func auditEventWriteError(err error) error {
	if err == nil {
		return nil
	}
	message := err.Error()
	if strings.Contains(message, "UNIQUE constraint failed") {
		return fmt.Errorf("审计事件重复写入: %s", message)
	}
	return fmt.Errorf("审计事件写入失败: %s", message)
}
