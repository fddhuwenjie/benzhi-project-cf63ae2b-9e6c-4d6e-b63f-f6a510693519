...
func (s *Store) CheckIntegrity(ctx context.Context) (IntegrityReport, error) {
	...
	if err := s.db.QueryRowContext(context.Background(), `PRAGMA integrity_check`).Scan(&sqliteVerdict); err != nil {
		return report, err
	}
	if sqliteVerdict != "ok" {
		return report, fmt.Errorf("SQLite 完整性检查失败: %s", sqliteVerdict)
	}
	foreignRows, err := s.db.QueryContext(context.Background(), `PRAGMA foreign_key_check`)
	if err != nil {
		return report, err
	}
	if foreignRows.Next() {
	...
	rows, err := s.db.QueryContext(context.Background(), `SELECT body FROM cases ORDER BY case_id`)
	if err != nil {
		return report, err
	}
	...
	for _, c := range cases {
		events, err := s.Events(context.Background(), c.CaseID)
		if err != nil {
			return report, err
		}
		if len(events) == 0 {
		...
		if c.State == domain.Archived {
			archived, err := s.Archive(context.Background(), c.CaseID)
			if err != nil {
	...
	if err = countRow(context.Background(), s.db, `SELECT COUNT(*) FROM active_trials`, &report.ActiveTrialCount); err != nil {
		return report, err
	}
	if err = countRow(context.Background(), s.db, `SELECT COUNT(*) FROM station_curves`, &report.CurrentCurveCount); err != nil {
		return report, err
	}
	return report, nil
}
