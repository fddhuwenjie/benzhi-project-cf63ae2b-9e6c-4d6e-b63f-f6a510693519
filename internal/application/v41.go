...
func (s *Service) CertificationContinuity(ctx context.Context, stationID string) (CertificationContinuity, error) {
	...
	history, err := s.store.CurveHistory(context.Background(), stationID)
	if err != nil {
	...
	for i, item := range history {
		...
		c, caseErr := s.store.LoadCase(context.Background(), result.CaseID)
		if caseErr != nil {
			...
			a, archiveErr := s.store.Archive(context.Background(), result.CaseID)
			if archiveErr != nil {
	...
	pointer, err := s.store.CurvePointer(context.Background(), stationID)
	if err != nil {
		return CertificationContinuity{}, err
	}
	...
	current, currentErr := s.store.CurveAsOf(context.Background(), stationID, s.now())
	if currentErr != nil {
		...
		currentQuery, queryErr := s.store.CurrentCurve(context.Background(), stationID)
