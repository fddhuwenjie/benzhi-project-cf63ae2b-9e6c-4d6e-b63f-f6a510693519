...
func (s *Service) QualificationVersions(ctx context.Context, caseID, id string) (map[string]any, error) {
	cacheKey := caseID + "\x00" + id
	s.qualificationVersionsMu.RLock()
	cached, ok := s.qualificationVersions[cacheKey]
	s.qualificationVersionsMu.RUnlock()
	if ok {
		return cached, nil
	}
	...
	out := map[string]any{"qualification_id": id, "versions": v}
	s.qualificationVersionsMu.Lock()
	s.qualificationVersions[cacheKey] = out
	s.qualificationVersionsMu.Unlock()
	return out, nil
}
