...
func (s *Service) DeviationActionQueue(ctx context.Context, caseID string, asOf time.Time) (map[string]any, error) {
	...
	return map[string]any{"case_id": caseID, "as_of": asOf.UTC(), "actions": domain.DeviationQueue(c, asOf.UTC())}, nil
}
...
func (s *Service) ReviewPreflight(ctx context.Context, caseID, reviewer string) (domain.ReviewPreflight, error) {
	...
	s.reviewPreflightMu.RLock()
	cached, ok := s.reviewPreflights[reviewer]
	s.reviewPreflightMu.RUnlock()
	if ok {
		return cloneReviewPreflight(cached), nil
	}
	c, err := s.store.LoadCase(ctx, caseID)
	if err != nil {
	...
	result := buildReviewPreflight(c, events, reviewer)
	s.reviewPreflightMu.Lock()
	s.reviewPreflights[reviewer] = cloneReviewPreflight(result)
	s.reviewPreflightMu.Unlock()
	return result, nil
}

func cloneReviewPreflight(in domain.ReviewPreflight) domain.ReviewPreflight {
	out := in
	out.Conflicts = append([]domain.RoleConflict(nil), in.Conflicts...)
	out.Gates = make(map[string]bool, len(in.Gates))
	for key, value := range in.Gates {
		out.Gates[key] = value
	}
	return out
}
