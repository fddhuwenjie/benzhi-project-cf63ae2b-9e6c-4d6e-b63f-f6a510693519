...
type Service struct {
	...
	caseReads  map[string]*caseReadFlight
}
...
func New(store *repository.Store) *Service {
	return &Service{store: store, engine: assessment.New(), now: func() time.Time { return time.Now().UTC() }, caseReads: map[string]*caseReadFlight{}}
}
...
func (s *Service) Get(ctx context.Context, id string) (*domain.Case, error) {
	...
	s.caseReadMu.Lock()
	if flight, ok := s.caseReads[id]; ok {
		s.caseReadMu.Unlock()
		<-flight.done
		return cloneCaseReadResult(flight)
	}
	flight := &caseReadFlight{done: make(chan struct{})}
	s.caseReads[id] = flight
	s.caseReadMu.Unlock()

	loaded, err := s.store.LoadCase(ctx, id)
	s.caseReadMu.Lock()
	flight.result = loaded
	flight.err = err
	close(flight.done)
	delete(s.caseReads, id)
	s.caseReadMu.Unlock()
	return cloneCaseReadResult(flight)
}

func cloneCaseReadResult(flight *caseReadFlight) (*domain.Case, error) {
	if flight.err != nil || flight.result == nil {
		return nil, flight.err
	}
	body, err := json.Marshal(flight.result)
	if err != nil {
		return nil, err
	}
	var cloned domain.Case
