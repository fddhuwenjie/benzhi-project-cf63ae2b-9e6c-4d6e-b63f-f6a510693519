...
func (s *Service) QualityPreflight(ctx context.Context, caseID string) (assessment.QualityReport, error) {
	s.qualityReportMu.RLock()
	cached, ok := s.qualityReports[caseID]
	s.qualityReportMu.RUnlock()
	if ok {
		return cloneQualityReport(cached), nil
	}
	c, err := s.store.LoadCase(ctx, caseID)
	if err != nil {
		return assessment.QualityReport{}, err
	}
	report := s.engine.EvaluateSamples(c.Evidence)
	s.qualityReportMu.Lock()
	s.qualityReports[caseID] = cloneQualityReport(report)
	s.qualityReportMu.Unlock()
	return report, nil
}

func cloneQualityReport(report assessment.QualityReport) assessment.QualityReport {
	report.Issues = append([]assessment.QualityIssue(nil), report.Issues...)
	report.BlockingRules = append([]string(nil), report.BlockingRules...)
	return report
}
...
func (s *Service) Diagnostics(ctx context.Context, caseID, band, verdict, side string) (DiagnosticResult, error) {
	...
	out := DiagnosticResult{MethodVersion: a.MethodVersion, InputDigest: a.InputDigest, Parameters: a.Parameters, Integrity: "pass", BandSummaries: []domain.ResidualBandSummary{}, Details: []domain.ResidualDetail{}, Influence: []domain.InfluenceDetail{}, FloodMarks: append([]domain.FloodMarkConstraint(nil), a.FloodMarkConstraints...), Boundaries: []domain.BoundaryDiagnostic{}}
	for _, x := range a.BoundaryDiagnostics {
		if side == "" || x.Side == side {
			out.Boundaries = append(out.Boundaries, x)
		}
	}
	for _, x := range a.BandSummaries {
		if band == "" || x.Band == band {
			out.BandSummaries = append(out.BandSummaries, x)
		}
	}
	for _, x := range a.ResidualDetails {
		if (band == "" || x.Band == band) && (verdict == "" || x.Verdict == verdict) {
			out.Details = append(out.Details, x)
		}
	}
	for _, x := range a.InfluenceDetails {
		if verdict == "" || x.Verdict == verdict {
			out.Influence = append(out.Influence, x)
		}
	}
	return out, nil
}
