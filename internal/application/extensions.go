...
func (s *Service) ReviewHistory(ctx context.Context, caseID string) ([]domain.ReviewRound, error) {
	if cached, ok := s.reviewHistory.Load(caseID); ok {
		return cached.([]domain.ReviewRound), nil
	}
	c, err := s.store.LoadCase(ctx, caseID)
	if err != nil {
		return nil, err
	}
	rounds := append([]domain.ReviewRound(nil), c.ReviewRounds...)
	s.reviewHistory.Store(caseID, rounds)
	return rounds, nil
}
