...
func (s *Service) BaselinePreflight(ctx context.Context, caseID string) (BaselinePreflight, error) {
	s.baselinePreflightMu.Lock()
	if cached, ok := s.baselinePreflights[caseID]; ok {
		s.baselinePreflightMu.Unlock()
		return cloneBaselinePreflight(cached), nil
	}
	s.baselinePreflightMu.Unlock()
	...
	if len(c.Evidence) == 0 && len(c.Qualifications) == 0 {
		s.baselinePreflightMu.Lock()
		s.baselinePreflights[caseID] = cloneBaselinePreflight(preflight)
		s.baselinePreflightMu.Unlock()
	}
	return cloneBaselinePreflight(preflight), nil
}

func cloneBaselinePreflight(in BaselinePreflight) BaselinePreflight {
	raw, _ := json.Marshal(in)
	var out BaselinePreflight
