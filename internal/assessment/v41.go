...
func (e *Engine) TrialProgressAtExpiry(c *domain.Case, decision domain.Decision) domain.TrialProgress {
	...
	for _, observation := range c.TrialObservations {
		...
		observation.CountsTowardProgress = observation.Verdict == "continue" && observation.RecordState != "superseded"
		for _, suspension := range c.TrialSuspensions {
			if suspension.Investigation != nil && within(observation.ObservedAt, suspension.Investigation.ImpactStartedAt, suspension.Investigation.ImpactEndedAt) {
				observation.CountsTowardProgress = false
			}
		}
